...
CI/agents). Publish with `--visibility private|org|public` — private (you and
the people you invite; for an agent publish, the user the agent acts on behalf
of owns it), org (your workspace), or public (the link works for anyone; add
`--password` to lock it). The default is `private`, so nothing is visible to
anyone else unless you say so. When `DERIVE_TOKEN` is set, gated artifacts 404
for anyone without a session or the token.
