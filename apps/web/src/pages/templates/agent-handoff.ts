...
Use the template as a reference, then make the decisions this brief needs:
1. ${confirmWorkspace} Use Derive's read tool to inspect the exact reference before creating anything.
2. Keep the useful structure, visual language, interactions, and narrative rhythm. If the artifact announces a Derive protocol or structured facts, read its matching Derive skill and validate that protocol before publishing. Adapt the content and other decisions to my brief. Use find when workspace evidence would improve the result.
