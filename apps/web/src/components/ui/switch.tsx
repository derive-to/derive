...
function Switch({
  ...
  return (
    <SwitchPrimitive.Root
      ...
      data-size={size}
      className={cn(
        // Track rest = quiet well with an inset hairline (the Nemonic track idiom); checked
        // commits to amber and drops the edge. The transparent border only preserves the
      ...
      {...props}
    >
      {/* Thumb: foreground at rest → charcoal on the amber track when checked (the brand-checked
          pairing — never white on amber). Only the translate transitions: movement is allowed. */}
