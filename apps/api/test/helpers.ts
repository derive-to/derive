...
const makePgStore = (name: string, users: TestUser[], team: Seat[]): TestStore => {
  ...
  const ready = (async (): Promise<TestStore> => {
    ...
    try {
      ...
      await boot.query(
        `CREATE TABLE IF NOT EXISTS ${schema}."user" (id text primary key, email text, name text, image text, username text, discoverable boolean, profession text, about text, houseStyle text)`,
      )
