...
describe("materializeEdits: quote-scoped edits (the inline editor's shape)", () => {
  ...
  it("applies typed text, custom width, reorder, and removal in one atomic HTML save", async () => {
  ...
  <article data-derive-node="c"><h2>C</h2></article>
    ...
    expect(out.content).toContain('data-derive-node="a" data-derive-width="68"')
