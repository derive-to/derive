...
// THE CODE SANDBOX — adversarial tests first, because this runs text a model wrote, and that
// model may have read a hostile web page. The code is assumed hostile, not merely buggy.
//
// Two properties carry everything, and both are asserted here rather than argued for:
//   1. The sandbox holds NO SECRETS (env: {}), so an escape finds nothing worth taking.
//   2. Tools are MARSHALLED, so code can do exactly what the caller could already do by hand.
//
// Node's `vm` is not a boundary on its own — `x.constructor.constructor` walks out to the host
// `Function` and from there to `process`. The worker thread is the real boundary; the prelude
// closes the known escape so the two have to fail together.
...
describe("code sandbox: it runs code", () => {
...
describe("code sandbox: it holds", () => {
