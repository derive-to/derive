  ...
  app.put("/v1/contexts/:id/runtime/schedule", async (c) => {
    ...
    const body = await readJson(
      ...
      z.object({
        ...
        cron: z.string().trim().min(1).max(128).nullable(),
    ...
    if (enabled && runtime.connection_id === null && ctx.deps.runtime) {
      try {
        if (
        ...
        return fail(c, 502, "Could not verify the workflow’s model account")
      }
    }
    let next: string | null
    try {
      next = body.cron === null ? null : nextRuntimeOccurrence(body.cron, body.timezone)
    } catch {
    ...
    return c.json({ schedule, next_run_at: enabled ? next : null })
  })
  return app
}
