...
export const workspaceRoutes = (ctx: AppContext) => {
  ...
  // The agent activity view (WP6): the workspace's recent runs, newest first.
  // Admin-only — it exposes what every agent did, why, and what it cost. A plain
  // route (not in the OpenAPI spec); the web activity page consumes it directly.
  app.get("/v1/workspace/agent-runs", async (c) => {
    const org = await requireWorkspace(c, "manage")
    if (org instanceof Response) return org
    const limit = Math.min(200, Math.max(1, Number(c.req.query("limit")) || 50))
    return c.json({ runs: await meta.listAgentRuns(org, limit) })
  })
