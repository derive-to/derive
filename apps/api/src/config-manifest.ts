...
const CONFIG_VARS: ConfigVar[] = [
  ...
  // -- search --
  {
    name: "DERIVE_EMBED_CF_ACCOUNT_ID",
    group: "search",
    doc: "Dense/semantic search (pgvector). Both DERIVE_EMBED_CF_ACCOUNT_ID and\nDERIVE_EMBED_CF_API_TOKEN enable it — embeddings are generated by Cloudflare Workers AI\n(bge-m3) over REST and stored in pgvector in your Postgres. Requires DATABASE_URL\n(Postgres); with the embedded-SQLite default it's skipped. Unset = lexical-only search.\nThe Cloudflare edge tier uses its Workers AI binding for embeddings instead of these vars.",
    example: "",
  },
...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    requires: ["DERIVE_EMBED_CF_ACCOUNT_ID", "DERIVE_EMBED_CF_API_TOKEN"],
    detail:
      "Dense/semantic workspace search: Cloudflare Workers AI (bge-m3) embeddings over REST, stored in pgvector in your Postgres and fused with lexical FTS. Also requires DATABASE_URL (Postgres) — with embedded SQLite it stays lexical-only.",
  },
]
