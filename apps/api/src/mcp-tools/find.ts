...
export function registerFindTool(tc: ToolContext): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "Find things in Derive — the MODE is decided by what you pass. Pass `short_id` + `query` to GREP within one artifact: matching lines with line numbers (in:'source'|'text', context lines, a past `version`), so you can then read a `lines` range or edit that spot. Pass `query` ALONE to SEARCH the whole workspace — artifacts ranked by relevance with a snippet each, so you find WHICH doc has something before opening it; this ALSO surfaces any askable context whose name matches. That search is LITERAL — send ONE keyword, never a whole question; empty means try another word or BROWSE, not that the workspace is empty (derive://skills/finding).Pass NEITHER to BROWSE the library: every artifact (short id, title, kind, is_skill, version, access, tags — skills:true or a `tag` narrows it) PLUS the askable contexts. Rows are typed (artifact | match | context); `read` a context row for its package, `use` it to give it work. Includes your own unlisted work. For the browse→work rhythm, read derive://skills/loop.",
