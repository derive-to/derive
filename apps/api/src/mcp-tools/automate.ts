...
// `list` remains a real action — list_automations routes through it — but only that tool
// passes a set containing it, so the write tool rejects it by name rather than hiding it.
const AUTOMATE_ACTIONS = ["create", "list", "run_now", "record", "create_context"] as const
const AUTOMATE_WRITE_ACTIONS = ["create", "run_now", "record", "create_context"] as const
...
const AUTOMATE_INPUT = z.object({
  ...
  action: z.string().describe(choiceDescription(AUTOMATE_WRITE_ACTIONS, "What to do.")),
...
/** The write tool infers this from AUTOMATE_INPUT; list_automations passes only `action`.
 *  Inferred rather than hand-written so the handler can never drift from the schema. */
type AutomateInput = z.infer<typeof AUTOMATE_INPUT>

export function registerAutomateTools(tc: ToolContext): void {
  const { server, ctx, agent, defaultOrg, scopeForCap, registered } = tc
  const meta = ctx.meta

  // Two tools over the one `run` below (declared after them; it is long and they are the
  // interesting part). `list` used to be an action alongside create/run_now/record, which
  // made the whole tool declare itself a write, so reading a workspace's standing jobs
  // prompted the user like scheduling a new one.
  //
  // Each tool passes the actions IT allows, so `automate` refuses `list` rather than
  // merely not advertising it — a split that only exists in a description is not a split.
  //
  // The owner gate is unchanged and still applies to the read: it sits above the action
  // branch, so listing has always required owner, and splitting a surface is not the
  // moment to widen who can see what a workspace runs.
  server.registerTool(
    "list_automations",
    {
      description:
        "List the automations and contexts this workspace has standing. Read-only; needs the same owner grant as wiring one. See derive://skills/loop.",
      annotations: {
    ...
    async () => run({ action: "list" }, AUTOMATE_ACTIONS),
  )
  ...
  server.registerTool(
    ...
    {
      description:
        "Scheduled and triggered work: create/run_now an automation, record an outcome, or create_context. See derive://skills/loop.",
      ...
      inputSchema: AUTOMATE_INPUT.shape,
    },
    async (input) => run(input, AUTOMATE_WRITE_ACTIONS),
  )

  async function run(input: AutomateInput, allowed: readonly string[]) {
    const wrongAction = badChoice("action", input.action, allowed)
