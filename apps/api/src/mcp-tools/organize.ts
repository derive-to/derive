...
/** Every argument the library surface accepts. The three tools below each expose a
 *  SUBSET of this as their own schema; one handler still routes on which arrived, so
 *  splitting the surface changed no behaviour. */
...
export function registerLibraryTools(tc: ToolContext): void {
  const { server, ctx, agent, actingFor, reach, resolveWs, wsArg } = tc

  // The library's findability metadata — tags, collections, shelving — as three tools over
  // one handler. This is the read/write carve-out mcp.ts describes: it was one `organize`
  // switching on which arguments arrived, which forced destructiveHint onto the whole tool
  // (because `state:'deleted'` is permanent) and so charged an approval prompt for reading
  // a tag vocabulary.
  //
  // `browse_library` reads, `organize` writes tags and collections, `shelve` owns the one
  // irreversible verb. Note this is a read/write split, NOT one tool per verb — the point
  // tools it once replaced (list_tags/suggest_tags/tag/list_collections/collect) are not
  // coming back, and mcp.test.ts still pins those names as gone.
  const run = async ({
    short_ids,
    add,
    remove,
    ...
    if (add || remove || set || collection || state) {
      if (!short_ids?.length) return err("Pass `short_ids` alongside the change you want to make.")
  ...
  const shortIds = (what: string) => z.array(z.string()).optional().describe(what)

  // READ. Genuinely read-only, so an annotation-honouring client runs it without a prompt
  // — the whole point of pulling it out of a destructive tool.
  server.registerTool(
    "browse_library",
    {
      description:
        "Read the library's findability metadata. No `short_ids` gives the workspace tag vocabulary and collections; with them, each artifact's tags and collections (plus suggestions for a single one). See derive://skills/organize.",
      annotations: {
      ...
      inputSchema: {
        short_ids: shortIds("Artifacts to inspect. Omit for the workspace overview."),
        workspace: wsArg,
      },
    },
    async ({ short_ids, workspace }) => run({ short_ids, workspace }),
  )

  // WRITE, reversible. destructiveHint:false is accurate here in a way it could never be
  // while `state` shared the tool: retagging and collecting are both undoable by doing the
  // opposite, and nothing on this schema can destroy anything.
  server.registerTool(
    "organize",
    {
      description:
        "Tag artifacts and fold them into collections. `set` replaces, `add`/`remove` adjust. Each artifact authorizes on its own, so untouchable ones come back skipped. See derive://skills/organize.",
      annotations: {
      ...
      inputSchema: {
        short_ids: shortIds("Artifacts to organize."),
    ...
    async ({ short_ids, add, remove, set, collection, workspace }) =>
      run({ short_ids, add, remove, set, collection, workspace }),
  )

  // WRITE, and the only irreversible verb on the surface. Kept as ONE parameter carrying
  // both directions: retire and restore are the same decision read two ways, and a surface
  // that can retire something without an obvious way back is the failure this avoids.
  // `deleted` rides along because it is the same authoring decision taken to its end, at a
  // higher role bar — but it is why this tool, and only this tool, is destructive.
  server.registerTool(
    "shelve",
    {
      description:
        "Retire artifacts from the library, restore retired ones, or delete permanently. state:'removed' is reversible and hands back its undo; state:'deleted' is not, and takes every version, comment and proposal with it. See derive://skills/organize.",
      annotations: {
        title: "Shelve or delete",
        readOnlyHint: false,
        destructiveHint: true,
        openWorldHint: false,
      },
      inputSchema: {
        short_ids: shortIds("Artifacts to shelve, restore or delete."),
        state: z
          .string()
          .describe(
            choiceDescription(
    ...
    async ({ short_ids, state, workspace }) => run({ short_ids, state, workspace }),
  )
}
