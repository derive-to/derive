...
4. Join the facts only by stable diagram/node IDs. Every visible node and edge must have exactly one
   ...
   result artifacts later. Never invent a placeholder artifact id. Give visible nodes a concise,
   editable `note` when the workflow `instruction` or `result` does not already explain what happens;
   human decision nodes normally need one.
