...
Use `await bugs.activity()` to read the latest attributed interactions:
...
- Commenters can add and update items and read activity.
- Editors and owners can interact too, and can also change the artifact source.
...
This is intentionally a small-state primitive, not a database API. Each key holds
an array of up to 2,000 JSON objects and 256 KB. Keys start with a letter and may
contain letters, numbers, `_`, or `-` (up to 64 characters). Updates use optimistic
concurrency so simultaneous votes are applied without silently overwriting one
another.
