  ...
  describe("pg store: user directory (Better Auth `user` table)", () => {
    ...
    it("browses discoverable people, resolves follower/following profiles, backfills author_id", async () => {
      await inSchema(
        async (boot, schema) => {
          await boot.query(
            `CREATE TABLE ${schema}."user" (id text primary key, email text, name text, image text, username text, discoverable boolean, profession text, about text, houseStyle text)`,
          )
          await boot.query(
