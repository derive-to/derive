...
4. **Review rounds** (the /derive loop): publish with `request_review:true`, then
   ...
   author, anchored or not), ack every human comment (a reaction at minimum),
   revise, and publish with `addresses` + `request_review:true` for the next round.
   The human never resolves threads — you settle thread state.
