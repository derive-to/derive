...
/** How a write to this target lands: publish live (with a review round) or file a
 *  proposal. Default propose — publishing is explicit per-target consent. */
export type WriteMode = "publish" | "propose"

export type Selector =
  | { kind: "artifact"; id: string; mode?: WriteMode }
  | { kind: "collection"; id: string; mode?: WriteMode }
  | { kind: "tag"; tag: string; mode?: WriteMode }
...
export const normalizeSelector = (v: unknown): Selector | null => {
  ...
  // Only the explicit publish opt-in is kept; anything else (absent, "propose", junk)
  // normalizes to the default so stored selectors stay canonical and minimal.
  const mode = o.mode === "publish" ? ({ mode: "publish" } as const) : {}
  if (o.kind === "artifact" && typeof o.id === "string" && o.id.trim() !== "")
    return { kind: "artifact", id: o.id.trim(), ...mode }
  if (o.kind === "collection" && typeof o.id === "string" && o.id.trim() !== "")
    return { kind: "collection", id: o.id.trim(), ...mode }
  if (o.kind === "tag" && typeof o.tag === "string" && o.tag.trim() !== "")
    return { kind: "tag", tag: o.tag.trim(), ...mode }
  return null
}
...
/** Per-write modes resolved from a target list: each artifact's own mode, and the mode
 *  for CREATED work (publish iff any collection/tag target explicitly says publish). */
export interface WriteModes {
  byArtifact: Record<string, WriteMode>
  create: WriteMode
}

export const writeModes = (sel: Selector[]): WriteModes => {
  const byArtifact: Record<string, WriteMode> = {}
  let create: WriteMode = "propose"
  for (const s of sel) {
    if (s.kind === "artifact") byArtifact[s.id] = s.mode ?? "propose"
    else if (s.mode === "publish") create = "publish"
  }
  return { byArtifact, create }
}
